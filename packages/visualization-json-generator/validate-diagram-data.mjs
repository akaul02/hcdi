 ...
 *
 * Usage: node packages/visualization-json-generator/validate-diagram-data.mjs path/to/diagram-data.json
 */
...
export function validateDiagramDataJson(parsed) {
  ...
  if (parsed.waterfallEvents !== undefined) {
    if (!Array.isArray(parsed.waterfallEvents)) {
      errors.push("waterfallEvents must be an array when present");
    } else {
      parsed.waterfallEvents.forEach((ev, i) => {
        const prefix = `waterfallEvents[${i}]`;
        if (!isRecord(ev)) {
          errors.push(`${prefix} must be an object`);
          return;
        }
        if (!Number.isInteger(ev.start) || ev.start < 0) {
          errors.push(`${prefix}.start must be a non-negative integer`);
        }
        if (!Number.isInteger(ev.end) || ev.end < 0) {
          errors.push(`${prefix}.end must be a non-negative integer`);
        }
        if (
          Number.isInteger(ev.start) &&
          Number.isInteger(ev.end) &&
          ev.end < ev.start
        ) {
          errors.push(`${prefix}.end must be >= start`);
        }
        if (typeof ev.label !== "string" || ev.label.trim().length === 0) {
